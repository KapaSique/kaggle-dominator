...
4. PARETO FRONT (rebuild nightly from fresh scores): from the COMPLETE scored submissions (skip pending/blank), build /tmp/<slug>_scores.json mapping each DISTINCT version label (read from the description) to its score — {\"label\":{\"public\":SCORE}, ...} — then run: python3 scripts/pareto_select.py /tmp/<slug>_scores.json --best \"<our current best label>\". This rebuilds the front from tonight's scores and marks which versions are DOMINATED (over-engineered — strictly beaten) vs the non-dominated front. If a front has fewer than 2 scored versions, skip Pareto for it and say so.

Then WRITE a dated digest to STATUS.md (OVERWRITE the whole file). One section per front containing: our best REAL score, our ladder position vs the top, deadline and days left, whether submissions look enabled, the PARETO verdict (non-dominated front vs dominated/over-engineered versions, and whether the tool says HOLD or REPLACE the best), and ONE concrete recommended next action. Be terse and factual: only numbers you actually read, never invented ones.
