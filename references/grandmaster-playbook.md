...
- 2026-06 — [code/optimization comp] **When the task decomposes into independent units, ensemble MULTIPLE public solutions by per-unit best-of — it beats any single base improved by your own refits.** neurogolf-2026 (minimize total inference cost, lower=better): one public base = 6275.09; your own holdout-validated conv-refits *on that single base* landed 6281–6287 (WORSE than the base); taking the per-task MINIMUM cost across several distinct public submissions (surgeon/galaxy2/multisrc/mirza) + lossless shrink passes reached 6239–6241 — the best honest result. Don't just improve one public solution; argmin across all of them per unit first, then refine.
- 2026-06 — [code/optimization comp] **Aggressive per-unit refitting can TOP the public board while overfitting private — gate every refit behind an honest holdout.** neurogolf-2026: 30 unconstrained single-Conv refits scored the best public number (6102 vs 6275 baseline) but overfit the private set and were reverted to the safe graph-surgery-only floor (6275.70); thereafter only holdout-validated refits were kept (6239–6287). A big public jump from many small per-unit fits is a private-overfit red flag, not a win.
