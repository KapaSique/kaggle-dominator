...
1. **Pareto-front, not a single BEST_KNOWN** (GEPA ParetoCandidateSelector, EvoSkill held-out frontier). Keep candidates non-dominated across pool-instances (opponents/seeds/folds/metric-axes), not just the average-best. A challenger may REPLACE the active best ONLY if it Pareto-dominates it (≥ every instance, > at least one). Tool: `scripts/pareto_select.py` (tested: it HOLDS pokemon-anchor-602 and discards search; v3-526 is co-optimal, never a replacement). This is the formal guard for the v3/search trap.

2. **Regime-aware lens** (harness-evolver σ-Select; A-EVO-Lab adaptive harness). The lever depends on the regime: WEAK score → tune prompt/params (cheap, in-regime); CEILING/PLATEAU → switch the CLASS of lever (new mechanism), do NOT keep tuning. A-EVO proved a single dense harness peaks early then declines — the scientific form of our #1 loss "over-engineering past the peak". At a plateau: change class, don't add complexity.

3. **Gaming/goodhart critic** (harness-evolver harness-critic; Kitchen Loop "converge to spec, not metric"). After any large jump an INDEPENDENT critic asks "did we game the proxy?" Our measured form: a local gain that doesn't transfer to the real ladder IS gaming (orbit/maze/pokemon lied 3×). Verify a jump on the REAL metric before trusting it.

4. **Promote-threshold compound learning** (harness-evolver `promote_learnings.py`). An insight enters the durable skill only after it's confirmed N times — not on first sighting. Stops one lucky/noisy run from rewriting doctrine. Curator harvests candidates; only repeatedly-measured ones get promoted.

5. **ρ/σ/ι/ε/κ loop + swarm option** (harness-evolver Autogenesis algebra; claude-flow hive-mind). The grind loop = Reflect (read real traces — why it failed) → Select (regime-aware lens) → Improve (N proposers, may self-abstain) → Evaluate (Pareto gate on held-out) → Commit (merge only a dominating winner). For multi-front parallel grind, a Queen→workers swarm (claude-flow) can run orbit+pokemon+JED concurrently — but ONLY after one local_grind loop is proven (never parallelize an unverified loop).

6. **Explicit agent roles for tabular** (AutoKaggle Reader/Planner/Developer/Reviewer/Summarizer; validated by NVIDIA's 2026 Kaggle win — 3 LLM agents, **850 experiments**, 4-level/150-model hill-climb stack = 1st place). Decompose a tabular push into roles: **EDA-Reader** (profile data, adversarial-validation train≠test) → **Planner** (CV scheme FIRST, then feature plan) → **Developer** (kernel batches, magic-feature search) → **Reviewer** (Pareto gate + CV↔LB gap check) → **Summarizer** (fold into BEST_KNOWN + winning-solutions ref). The decisive lever NVIDIA confirmed is **scale** (hundreds of experiments + hill-climbing stacking on cuML), not a smarter single model. Mirror [[winning-solutions]] for recon material.
...
- 2026-06 — [any comp, process] **Check submissions are ENABLED before investing a session — the listed "deadline" is often the END/scoring date, not the submission deadline.** maze-crawler: `kaggle competitions list` showed 2026-06-30 and the UI said "20 submissions remaining today", yet every submit 400'd because **submissions were disabled** (real window closed ~06-17; 06-30 was final-scoring). A whole build-validate-breakthrough cycle (v23ram +7.1%) couldn't be submitted. RECON step 0: probe a throwaway submit / read the UI for "submissions disabled" FIRST — especially simulation/agent comps that close early.
...
- 2026-06 — [code comp] **Recipe QUALITY beats N VOLUME — a smaller, better-crafted submission can crush a larger crude one.** ai-agent-security: the public guide24 recipe at N=350 scored 30.53, beating our own v8 mass at N=900 (18.11). Fewer candidates, more severity/cells per finding. Don't just scale N — find the recipe the top uses, then scale.
- 2026-06 — [code comp, hidden private] **A public-LB plateau can mean your attack only works on the PUBLIC guardrail — the private split blocks it.** ai-agent-security: our exfil-marker was DENY'd by a reconstructed private guardrail (ContentScan), so 18.11 was public-only (private≈0), while a confused-deputy variant passed ALL guardrails. When public stalls and the top is far ahead on a hidden private, check whether your mechanism even transfers to the private regime.
- 2026-06 — [process/tooling] **Parsing `kaggle competitions submissions` text: numbers in the DESCRIPTION column get mis-read as scores — use CSV/positional parsing.** Two independent sessions both grepped "33.82 / 53.80" as public scores when those were just "target ~55.8 / ~36-40" text in the message; the only real score was 30.530. Extract publicScore by column position (after the status field), never a loose number regex over the whole line.
...
- 2026-06 — [simulation/ladder, imitation] **Behavior-cloning a heuristic teacher REPRODUCES its strength, never exceeds it — clone ≈ teacher minus imitation loss. To break a plateau you need search/RL ABOVE the teacher, not a faster copy of it.** orbit-wars: a PointNet BC'd on 28k v6/lookahead decisions reached val_acc_target=0.909, then gated *dead even* with v6 (2p 54% over 48 games = not significant; 4p table rank 1.50=1.50, exactly 6-6). It learned to be v6, so it can't beat v6 — and you don't swap a proven BEST (real-ladder 737.9) for a locally-equal unproven clone (pure risk, no edge). BC is worth it only for *speed* (cheap inference replacing an expensive planner at equal strength) or as a *warm-start* for RL that then trains past the teacher. NB: the RL-past-teacher path (PPO self-play) is what could exceed it — but here that kernel hit the time limit (CANCEL) and didn't finish.
...
- 2026-06 — [simulation/ladder] **Run a FRESH death-cause diagnostic BEFORE building any survival fix — memory about "the bottleneck" goes stale or was for a different opponent.** maze: I built a late-game-scroll fix from a memory note, but a 5-second `death_causes.py` showed matches vs the sprinter end at median step 260 (0% reach the end) with **62% deaths = factory_ram** — the scroll fix targeted a regime the match never reaches. Diagnose the actual cause on the actual current opponents first; don't code a fix for a remembered bottleneck.
- 2026-06 — [simulation/ladder] **Your local opponent panel may not exercise the very mechanic you're fixing — then it can't validate the fix.** maze: a late-game scroll-survival fix (NORTH-only JUMP-conserve + bigger emergency_gap) measured 71.2% vs v18's 72.5% on the panel — the JUMP-restrict *hurt* (lateral jumps are needed to escape walls), and the gap-change was a flat 0 because the panel (LB1223/v21/v9/starter) never reaches the fast-late-scroll edge where it matters. Don't submit it on faith (rule 4); either add a panel opponent that forces the late-game regime, or accept you can't locally validate that lever.
- 2026-06 — [simulation/ladder] **Isolating ONE component of a failed bundle can be the win — test it alone.** maze: the v20 bundle (2 scouts + ceiling_buffer) overfit and lost on the ladder, so the whole thing was filed as failed. But ceiling_buffer ISOLATED on v18 (factory stays 1 row below the north wall → maneuver room vs the enemy-factory ram) measured 79.9% vs v18's 72.8% on the panel (+7.1%), and crucially +29% vs the econ-bot (39→68%) — the exact opponent class that beat v18. ram-deaths 62→56%. When a bundle fails, A/B each component alone before discarding the idea.
- 2026-06 — [simulation/ladder] **"Idea failed" vs "my implementation failed" — don't bury a lever the leader is winning on.** maze-crawler: my mine-economy agents (v19-21) scored *below* the simple v18, so I filed economy as "over-engineering". But the #1 player (Genematon, 2109 vs my 1015) wins *on mine-economy* — the idea is the path to the top; my implementation was just bad. Before abandoning a complex lever, check whether the leaderboard top is using it: if they are, fix your implementation, don't drop the lever.
...
- 2026-06 — [simulation/agent, MEASURED] **Naive forward-search/lookahead can CATASTROPHICALLY hurt a strong heuristic agent — measure before enabling, don't assume "search = lever".** PTCG (cabt): a corrected 1-step-lookahead + greedy-completion wrapped around the organizers' tuned heuristic scored −34 (vs random) and −53 (vs Crustle) BELOW the same heuristic with search OFF, at ~15× the latency (clean N=150 A/B, 0 errors). Root cause: the heuristic already did sophisticated 1-ply attack planning, while the search's leaf eval (evaluate_state) was CRUDER than the heuristic's own scoring → search picked first-actions by a worse metric (redundant-to-worse), plus global-state (plan/pre_turn) contamination from calling the stateful heuristic on simulated states. Confirming signal: the strongest PUBLIC agent (romanrozen LB860) ships USE_SEARCH=False. Contrast orbit-wars where exact-sim+rollout gave +165 — there the base policy was a weak greedy heuristic, so search ADDED planning; here the base WAS the planner. Lesson: search only helps when its leaf eval ≥ the base policy's own reasoning; wrapping a strong 1-ply heuristic in a search with a weaker eval is over-engineering past the peak. Validate on the pool BEFORE spending a ladder slot — the arena caught it, no submit wasted.
